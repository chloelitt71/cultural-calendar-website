  ...
  return (
    <div className="page-transition section-shell pb-20 pt-8">
      <header className="mb-8 max-w-3xl">
        <p className="font-mono text-[10px] uppercase tracking-[0.28em] text-[#c84c2f]">Talent</p>
      ...
      <div className="mb-6 space-y-4 rounded-[1.25rem] border border-white/8 bg-black/20 p-4 md:p-5">
        <div className="inline-flex flex-wrap gap-1 rounded-xl border border-white/10 bg-white p-1">
          {SEGMENTS.map((s) => (
            <button
              key={s.id}
              type="button"
            ...
            {PR_FILTERS.map((f) => (
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                  prFilter === f.id ? 'bg-white/12 text-zinc-100 ring-1 ring-[#c84c2f]/35' : 'text-zinc-500 hover:bg-white/6 hover:text-zinc-300'
                }`}
              >
                {f.label}
              </button>
